ancova.multcomp <-
function (model) {
  if (ncol(model.frame(model))!=3) {
    stop("this function is designed only for models containing\n   1 covariate and 1 factor")
  }
  ...
  dotchart(pred$fit,xlim=c(0.9*min(pred$fit-ci),1.1*max(pred$fit+ci)),labels=levels(fact),
    pch=16,xlab=colnames(m.frame)[1],main="Adjusted means +/- 95% confidence interval")
