...
function (x,data,mu=NULL) {
  if (is(x,"formula")) {
    ...
    if (!is.ordered(resp)) {resp <- factor(resp,ordered=TRUE)}
    resp2 <- as.numeric(resp)
    fact <- interaction(mf[,2:ncol(mf)],sep=":")
    ech1 <- resp2[as.numeric(fact)==1]
    ech2 <- resp2[as.numeric(fact)==2]
	...
	if (length(mu2)==0 || is.na(mu2)) {stop(paste0("'mu' is not a level of ",dname))}
	signs <- x2-mu2
	p <- binom.test(length(signs[signs>0]),length(signs[signs!=0]),0.5)$p.value
	med <- median(x)
	names(med) <- paste0("median of ",dname)
	names(mu) <- "median"
    }
  }
